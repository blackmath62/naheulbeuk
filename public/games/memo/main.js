...
function afficherTableau() {
    ...
    for (var i = 0; i < tabJeu.length; i++) {
        ...
        for (var j = 0; j < tabJeu[i].length; j++) {
            if (tabJeu[i][j] === 0) {
                txt += "<button class='btn btn-primary m-2 text-center' style='width:70px; height:70px' onClick='verif(\"" + i + "-" + j + "\") '>Afficher</button>";
            } else {
                txt += "<img src='" + getImage(tabJeu[i][j]) + "' style='width:70px;height:70px' class='m-2'>"
            }
        }
        txt += "</div>";
    }
